...
Task6：
现在，我们假设对每一个stage = 3的流程，都有一系列的审核结果。我们希望把所有审核结果和意见，包括给出审核员意见的等级，审核员通过与否的决策，以及该客户风险问卷信息和其他基本信息，以及你认为其他有用的信息打包成一条消息，发送到消息队列(kafka)中。

你现在需要先实现：修改submitAuditResult任务结果的一些处理逻辑。当得出最终审核结果时（需要根据之前实现的规则判断是否得到最终审核结果，注意审核不通过也是最终结果），除了更新audit_log，还需要收集有用的信息，打包成一条消息，发送到消息队列中。
你需要设计消息的结构。不需要生成单测。你只需要新增这个功能。代码必须精确、模块化、可测试。如果有新增依赖，请修改pom文件。
