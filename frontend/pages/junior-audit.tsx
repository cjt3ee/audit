...
import { getMergedTasks, submitAuditResult, getAuditorHistory, removeCompletedTask, formatPhone, formatDateTime, getRiskTypeBadgeClass, getStageText, getMaxLossText, validateAuditForm, releaseAuditTask, getNewAuditTasks } from '../utils/auditorApi';
...
const JuniorAuditPage: React.FC = () => {
  ...
  const checkForNewTasks = async () => {
    ...
    try {
      const currentTaskIds = tasks.map(task => task.auditId);
      const response = await getNewAuditTasks(0, auditorInfo.auditorId, currentTaskIds);
